...
for temperature in [25,50]:
    for terminalv in [0.9, 1.0, 1.1]:
        ...
        for P_Gen_1 in numpy.arange(0, S_1 * derate + 0.01, 2):
            ...
            Q_Gen_1 = max(-50.16, -math.sqrt(max((S_1 * S_1 * derate * derate - P_Gen_1 * P_Gen_1),0)))
            Q_Gen_BESS = max(-9.26, -math.sqrt(max(S_BESS * S_BESS * derate * derate - P_BESS * P_BESS,0)))
        ...
        for P_Gen_1 in numpy.arange(0, S_1 * derate + 0.01, 2):     #################################CHANG: DO NOT CHANGE INDENTATION
            ...
            ierr = psspy.fdns([1, 0, 0, 1, 1, 0, 99, 0])
            P_BESS = P_Gen_1 / (S_1 * derate + 0.01) * P_BESS_max
            Q_Gen_1 = min(50.16, math.sqrt(max((S_1 * S_1 * derate * derate - P_Gen_1 * P_Gen_1),0)))
            Q_BESS = min(9.26, math.sqrt(max(S_BESS * S_BESS * derate * derate - P_BESS * P_BESS,0)))
    ...
    #                            [_f, _f, _f, _f, _f, _f, _f, _f, _f, _f, Capacitor, _f], "")
            ...
            if valid_pq == 1:
                ...
                V_Record.append(terminalv)
                print overloop
                # print str(pval); print str(P_Gen_1);
                # print str(qval); print str(Q_Gen_1);
                # print str(terminalv)
        print overloop
#     print str(P_Record)
#     print str(Q_Record)
...
PQ_Angle = numpy.arccos(numpy.divide(PQ_Array[0], numpy.sqrt(numpy.square(PQ_Array[0]) + numpy.square(PQ_Array[1]))));
A_40_09_index = numpy.argsort(PQ_Angle / 83.6);  # MAY NEED TO CORRECT 150 TO 400.

PQ_Array = [Q_40_10 - DrawCenterX, P_40_10 - DrawCenterY];
PQ_Angle = numpy.arccos(numpy.divide(PQ_Array[0], numpy.sqrt(numpy.square(PQ_Array[0]) + numpy.square(PQ_Array[1]))));
A_40_10_index = numpy.argsort(PQ_Angle / 83.6); # MAY NEED TO CORRECT 150 TO 400.

PQ_Array = [Q_40_11 - DrawCenterX, P_40_11 - DrawCenterY];
PQ_Angle = numpy.arccos(numpy.divide(PQ_Array[0], numpy.sqrt(numpy.square(PQ_Array[0]) + numpy.square(PQ_Array[1]))));
A_40_11_index = numpy.argsort(PQ_Angle / 83.6);

PQ_Array = [Q_50_09 - DrawCenterX, P_50_09 - DrawCenterY];
PQ_Angle = numpy.arccos(numpy.divide(PQ_Array[0], numpy.sqrt(numpy.square(PQ_Array[0]) + numpy.square(PQ_Array[1]))));
A_50_09_index = numpy.argsort(PQ_Angle / 83.6);

PQ_Array = [Q_50_10 - DrawCenterX, P_50_10 - DrawCenterY];
PQ_Angle = numpy.arccos(numpy.divide(PQ_Array[0], numpy.sqrt(numpy.square(PQ_Array[0]) + numpy.square(PQ_Array[1]))));
A_50_10_index = numpy.argsort(PQ_Angle / 83.60);

PQ_Array = [Q_50_11 - DrawCenterX, P_50_11 - DrawCenterY];
PQ_Angle = numpy.arccos(numpy.divide(PQ_Array[0], numpy.sqrt(numpy.square(PQ_Array[0]) + numpy.square(PQ_Array[1]))));
A_50_11_index = numpy.argsort(PQ_Angle / 83.6);
...
P_40_09_plot= P_40_09; P_40_10_plot= P_40_10; P_40_11_plot=P_40_11;
P_40_09_plot[P_40_09_plot>65]=65; P_40_10_plot[P_40_10_plot>65]=65; P_40_11_plot[P_40_11_plot>65]=65;
...
CurrentAx.legend([r"""T=25,V=0.9""", r"""T=35,V=1.0""", r"""T=35,V=1.1""", r"""Automatic Access Standard"""])
...
# # q_low=-0.395*p_rate;
# # q_up = 0.395*p_rate;
# P_50_09_plot= P_50_09; P_50_10_plot= P_50_10; P_50_11_plot=P_50_11;
# P_50_09_plot[P_50_09_plot>65]=65; P_50_10_plot[P_50_10_plot>65]=65; P_50_11_plot[P_50_11_plot>65]=65;
#
#
# CurrentAx.plot(Q_50_09,P_50_09_plot,color='blue',linestyle='-',marker='o',markevery=5);
# CurrentAx.plot(Q_50_10,P_50_10_plot,color='green',linestyle='-',marker='s',markevery=5);
# CurrentAx.plot(Q_50_11,P_50_11_plot,color='orange',linestyle='-',marker='^',markevery=5);
# CurrentAx.plot([-25.67, -25.67, 25.67, 25.67], [0, 65, 65, 0], color='red', linestyle=':');
